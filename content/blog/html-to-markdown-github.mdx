...
- **Merged-cell tables.** `colspan`/`rowspan` from a source table flattens during conversion, since GFM tables can't represent merged cells — rebuild these manually rather than trying to coax the converter into preserving them.
...
Links convert with their `href` intact, but if your old HTML used manually assigned `id` attributes for in-page jump links, you'll need to update them — GitHub auto-generates heading anchor IDs from the heading text.
