...
|---|---|---|
| `<p style="color:#333;font-size:14px">Text</p>` | `Text` | Inline style attributes are dropped entirely, regardless of what CSS they specify — there's no Markdown syntax to preserve them in |
| `<span style="color:red">Text</span>` (no `<strong>`/`<em>` inside) | `Text` | Redundant spans with no semantic tag collapse to plain text — the color attribute and wrapper are discarded |
...
- **Merged-cell tables.** Tables pasted from Word or Excel frequently use `colspan`/`rowspan` for visual layout. These flatten during conversion since GFM tables can't represent merged cells — expect to rebuild complex tables manually.
...
- **Verify every table.** Tables are the highest-risk element in Word- or Confluence-style messy HTML — check each converted table specifically before trusting the rest of the document.
...
If Word applied a heading *look* using font size and bold styling rather than an actual Heading 1/2/3 style, the exported HTML contains a styled `<p>` or `<span>`, not a real `<h1>`–`<h6>` tag — and the converter can only convert what's actually marked up as a heading. Apply real heading styles in Word before exporting, or add the `#` syntax manually afterward.
