---
title: "Why Markdown Tables Break in PDF Exports"
description: "Markdown tables are a common cause of broken PDF exports. Learn why they fail, what correct output looks like, and how to fix every table rendering problem."
datePublished: "2026-03-08"
tags: ["markdown", "pdf", "tables", "github-flavored-markdown", "developer-tools"]
image: "/blog/markdown-table-pdf.jpg"
imageAlt: "Performance analytics dashboard with charts and data on a laptop screen"
...
The Markdown spec doesn't define table syntax. Tables are a [GitHub Flavored Markdown (GFM)](https://github.github.com/gfm/#tables-extension-) extension, so the `| Col | Col |` pipe syntax only works if the parser has GFM mode enabled. Parsers that implement only [CommonMark](https://commonmark.org/) (the base spec) will ignore the pipe characters entirely and render your table as a paragraph of `|` characters.

**Broken output:**

```
| Feature | MDTool | Others | | --- | --- | --- | | Tables | Yes | No |
```
...
```
| Tool | Free | Code Highlighting | Mermaid | |---| --- | --- | --- | | MDTool | Yes | Yes | Yes | ...
```
...
## How MDTool Handles Markdown Table to PDF Correctly

MDTool avoids all three failure modes:
...
Fix: Your converter doesn't support GFM. Switch to MDTool, which has GFM enabled by default.

**Problem: Table renders in the preview but collapses in the PDF**

Fix: This is the CSS race condition problem. The converter captures the page before styles load. MDTool inlines all CSS to avoid this.

**Problem: Table columns are unequal widths**

Fix: This is normal behavior, since GFM tables don't require equal column widths. If you want consistent column widths, set explicit widths in a `<style>` block above the table (MDTool's Markdown parser allows inline HTML).
...
Inline code in table cells (e.g., `` | `const x = 1` | ``) renders with a gray background and monospace font in the cell. This works correctly in MDTool. Just make sure the backtick pairs are balanced.
...
For a full side-by-side comparison including code blocks and Mermaid support, see [Best Markdown to PDF Converter in 2026](/blog/best-markdown-to-pdf-converter). For the full table syntax reference alongside every other Markdown element, see the [Markdown cheatsheet](/blog/markdown-cheatsheet).

---

## Frequently Asked Questions

**Q: Does MDTool support every GFM table alignment option?**
...
No. GFM table syntax doesn't support cell merging. Merged cells require raw HTML (`<table>` with `colspan`/`rowspan` attributes). MDTool's Markdown parser allows inline HTML, so you can use a raw `<table>` tag for complex layouts.
...
Yes. MDTool's theme CSS uses `border-collapse: collapse` and explicit `border` declarations that are print-safe. The light gray backgrounds on headers and alternating rows use colors that print visibly on monochrome printers.
...
There's no hard limit. MDTool has handled tables with hundreds of rows. For very large tables, the PDF will span multiple pages automatically, and the page break logic handles table splitting.
