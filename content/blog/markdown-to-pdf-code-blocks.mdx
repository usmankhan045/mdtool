---
title: "Markdown to PDF Code Blocks: Why They Break"
...
image: "/blog/markdown-to-pdf-code-blocks.jpg"
imageAlt: "Blurred close-up of syntax-highlighted code on a dark screen"
...
- Monospace font not used, so code renders in the same proportional font as body text
- No syntax highlighting, so everything is plain black text
...
```javascript
// MDTool API example — fetch and convert Markdown to PDF
...
If the preview looks right, the PDF will match, because the converter uses the same rendered HTML for both.
...
Click **Download PDF**. The conversion happens entirely in your browser using `html2pdf.js`, so your Markdown never leaves your machine, and no server processes it.
...
Fix: Make sure your code lines are under ~80 to 90 characters. The converter wraps very long lines, but it can't reformat code logic. Break long lines manually before converting.
...
## How MDTool Handles Code Blocks Differently
...
MDTool takes a different approach. The entire conversion runs **client-side in your browser** using `html2pdf.js` with `html2canvas`. This means:
...
For GitHub READMEs specifically, check out our guide on [converting GitHub READMEs to PDF](/blog/github-readme-to-pdf). It covers badge handling and image-heavy READMEs alongside code blocks. If you need a refresher on fenced code block syntax itself, see the [Markdown cheatsheet](/blog/markdown-cheatsheet).

## Frequently Asked Questions

**Q: Does MDTool support syntax highlighting for all programming languages?**

It supports the most common ones: JavaScript, TypeScript, Python, Bash, JSON, HTML, CSS, SQL, Go, Rust, and more. The underlying parser uses [`marked`](https://marked.js.org/) with [`highlight.js`](https://highlightjs.org/) for token coloring. If your language isn't highlighted, formatting (monospace font, background, indentation) is still preserved, and only color tokens are skipped.
...
There's no server-side limit because conversion happens in your browser. The practical limit is your device's RAM, and most modern machines handle Markdown files up to several megabytes without issue.

**Q: Can I use MDTool for confidential code?**
